...
const activate = () => {deactivated = true}
const attiva = () => {disattivo = true}
const chiudilo =() => {writeMail = !writeMail}
const submitOpen = () => {writeMail = true}
if (Object.keys(sessionStorage).includes('saltaLogin') ) {disattivo = true 
  attivo = false}
...
const tornaIndietro = (e) => {apriGrande = !e.detail}
...
const aggiungiAdInviati = (e) => {objects = e.detail}

const attivaAggiungi = (e) => {attivazione = e.detail.aggiungi}
...
const cambiaInviati = (e) => {tuttiInviati = false}
...
const inputDatas = (e) => {valore = e.detail}

const attivaPreferiti = (e) => {preferiti = e.detail}
...
const riduzione = (e) => {riduzioneLeftBar = e.detail}


const allungaWhitespaceEmailContent =(e) => {allungamento = e.detail}
...
{#if second == false}
  <div class="Log" class:deactivated={deactivated} >
    <EmailLogin  on:submit= {activate} on:submit|once={setTimeout(() => {
    second = true }, '1500')} />
  </div>
{:else if second == true}

  <div class="Log-pass" class:disattiva={disattivo}>
    <PasswordLogin on:submit= {attiva} on:submit= {
    setTimeout(() => {
    attivo = false
    }, '1500')
  }/>
...
<div class="App">
  ...
  <div class="corpo">
    <LeftSide on:click={submitOpen}   on:tornaAdHome={tornaAdHome}      on:vaiAdInviati={vaiAdInviati } on:attivare={ attivaPreferiti}  {apriGrande} {cambiaInvioArrivo} {preferiti} {tuttiInviati} {preferitiGrande} {riduzioneLeftBar} />
    <div class="email">
      {#if apriGrande == false && cambiaInvioArrivo == false && preferiti == false && preferitiGrande == false}
        <EmailBig name = {namey} {surname} {phoneNumber} {city} {state} {title} {loginUUID} {picture} {username} {ore} {minuti} {email}  {allungamento}  {tutto}  on:tornaIndietro={tornaIndietro} />
      ...
      {:else if preferitiGrande == true }
        <PreferitiGrande {nameo} {surnameo} {cityo} {phonenumbero} {stateo} {titleo} {loginUUIDo} {pictureo} {usernameo} {oreo} {minutio} {emailo} {allungamento}  on:tornareIndietroo={tornareIndietroo} />

      {:else if  cambiaInvioArrivo == true }
        {#if tuttiInviati == true}
          <Inviati {objects} {attivazione} {valore} {allungamento} on:cambiaInviati={cambiaInviati} on:aggiungiElementi={dede} on:aggiungiElementis={dedes}/>

        {:else}
          <SingkeSent {oggetto} {destinatario} {messaggio} {dates}          {oggettii} {destinatarioo} {messaggioo} {idd} {datess} {emaill} {loginUUIDs} {kiave}       on:tornaIndietr={tornaIndietr} {allungamento}  /> 
