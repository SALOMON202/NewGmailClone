...
const favourites = () => {select = !select}
...
<div class="sent">
  ...
  <div class='sent-header'>
    <div class="first-header">
      ...
      <div class="fourth-sent-elem"></div>
    ...
    <div class="contieni-mail">
      {#each chiavi as chiave }
        {#if chiave.includes('speciale') }
          <nobr>
            <div id="email--container"  class:visible={JSON.stringify(JSON.parse(localStorage.getItem(`${chiave}`))[1].email).includes( valore) ? true : false } class:invisible={JSON.stringify(JSON.parse(localStorage.getItem(`${chiave}`))[1].email).includes( valore) ? false : true } on:mouseenter={()=> {
            value = JSON.parse(localStorage.getItem(`${chiave}`))[0]
            if (chiave === `speciale${JSON.parse(localStorage.getItem(chiave))[0]}` ) {
            toggleEmail = true}
            }} 
            on:mouseleave={()=> {if ( chiave.includes(JSON.parse(localStorage.getItem(`${chiave}`))[0]) ) {
            value = ''
            toggleEmail = false}}}
            ...
            () => {  let cambiare = true
              ...
              let ore = 5
              let minuti = 5
              let persona = {name, surname, title , city, state , phoneNumber , loginUUID ,username , email, picture,ore , minuti, cambiare}
              dispatch('changepreferiti', persona)}}>

    <div class="contenitore-piccolo">
      <div class="preferiti-div"></div>

        <div class="star" class:selected ={select} on:click = {favourites} on:click={console.log(chiave)} ></div>
          <div class="prima-pt">
        <span class='received-email frst'>{JSON.parse(localStorage.getItem(`${chiave}`))[1].login.username}</span>
        <span class='received-email scnd'>{JSON.parse(localStorage.getItem(`${chiave}`))[1].login.uuid}</span>
        <div class="email-text">
          <p>Salve, sono {JSON.parse(localStorage.getItem(`${chiave}`))[1].name.title} {JSON.parse(localStorage.getItem(`${chiave}`))[1].name.first} {JSON.parse(localStorage.getItem(`${chiave}`))[1].name.last} e vorrei collaborare con voi. Vivo a {JSON.parse(localStorage.getItem(`${chiave}`))[1].location.city}, in {JSON.parse(localStorage.getItem(`${chiave}`))[1].location.country} ma sono disponibile anche a lavoro da remoto.
          Chiamate pure al numero {JSON.parse(localStorage.getItem(`${chiave}`))[1].phone}</p>
        </div> 
      </div>
    </div>


  <div class="whitespace-due" class:white-toggle={ chiave === `speciale${value}` ? toggleEmail : console.log('nonoe')}  class:allungaWhitespace = {allungamento} class:WhitespaceNormale = {!allungamento}> 
    <span class='dotss' class:dotss-toggled={ chiave === `speciale${value}` ? toggleEmail : console.log('nono')}>...</span>
    <span class='dob' class:showin={ chiave === `speciale${value}` ? toggleEmail : console.log('nono')} >{hour}:{minutes}</span>
    <div class="toggled-content" >
      <div class="first oo" class:showing={ chiave === `speciale${value}` ? toggleEmail : console.log('nono')}>
        <div class="first-img w"></div> 
        <div class="black-box">
        <nobr>Archivia</nobr>
        </div>
      </div>
      <div class="second oo" class:showing={chiave === `speciale${value}` ? toggleEmail : console.log('nono')}>
      ...
      <div class="fourth oo" class:showing={chiave === `speciale${value}` ? toggleEmail : console.log('nono')}>
  ...
  <div class="right-Space-shrunk"></div>
    </div>
  </nobr> 
  {/if}
  {/each}
...
.dotss-toggled{position: relative;
right:26px}

.white-toggle{min-width:250px;}

.showin{visibility:hidden}
